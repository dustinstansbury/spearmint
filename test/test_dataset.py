import pytest
from spearmint.dataset import Dataset, DataFrame
...
def test_default_init(proportions_data_small):
    ...
    assert (
        dataset.__repr__()
        == "Dataset(measures=['metric'], attributes=['attr_0', 'attr_1'])"
    )
...
def test_segments(proportions_data_small):
    dataset = Dataset(df=proportions_data_small, attributes=["attr_0", "attr_1"])
    ...
    proportions_data_small.columns = [
        c.replace("_", " ") for c in proportions_data_small.columns
    ]
    dataset = Dataset(df=proportions_data_small, attributes=["attr 0", "attr 1"])
