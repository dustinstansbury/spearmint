...
from spearmint.inference.bayesian import get_stan_model_code
from spearmint.inference.bayesian.models.binary import (
    beta_binomial,
    binomial,
    bernoulli,
)
from spearmint.inference.bayesian.models.continuous import gaussian, exp_student_t
from spearmint.inference.bayesian.models.counts import gamma_poisson
...
def test_bayesian_gaussian(means_data):
    ...
    test = HypothesisTest(
        inference_method="gaussian", metric="metric", control="A", variation="F"
    )
...
def test_bayesian_bernoulli(proportions_data_small):
    ...
    test = HypothesisTest(
        inference_method="bernoulli", metric="metric", control="A", variation="F"
    )
    inference_kwargs = dict(inference_method="sample")
    test_results = exp.run_test(test, inference_kwargs=inference_kwargs)
    assert pytest.approx(test_results.prob_greater, rel=0.1, abs=0.01) == 1.0
...
def test_bayesian_binomial(proportions_data_large):
    ...
    test = HypothesisTest(
        inference_method="binomial", metric="metric", control="A", variation="C"
    )
    inference_kwargs = dict(inference_method="sample")
    test_results = exp.run_test(test, inference_kwargs=inference_kwargs)
    assert pytest.approx(test_results.prob_greater, rel=0.1, abs=0.01) == 1.0
...
def test_empty_observations_exception(proportions_data_large):
    ...
    with pytest.raises(ValueError):
        test = HypothesisTest(
            inference_method="binomial", metric="metric", control="A", variation="F"
        )
        inference_kwargs = dict(inference_method="sample")
